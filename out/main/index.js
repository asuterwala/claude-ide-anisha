...
async function getGitFileStatuses(projectPath) {
...
let nextId = 1;
function createPtySession(projectPath, window) {
  const id = `pty-${nextId++}`;
...
function destroyAllPtySessions() {
...
function startWatching(projectPath, window) {
  stopWatching();
...
const sessionsFile = () => sysPath.join(electron.app.getPath("userData"), "recent-sessions.json");
async function loadRecentSessions() {
  try {
    const data = await promises$1.readFile(sessionsFile(), "utf-8");
    return JSON.parse(data);
  } catch {
    return [];
  }
}
async function saveRecentSessions(sessions2) {
  await promises$1.writeFile(sessionsFile(), JSON.stringify(sessions2, null, 2), "utf-8");
}
function registerIpcHandlers() {
  electron.ipcMain.handle("fs:readDir", async (_, dirPath) => {
    ...
    for (const entry of entries) {
      ...
      const fullPath = sysPath.join(dirPath, entry.name);
      const info = await promises$1.stat(fullPath).catch(() => null);
      nodes.push({
        name: entry.name,
        path: fullPath,
  ...
  electron.ipcMain.handle("git:status", async (_, projectPath) => {
    const statuses = await getGitFileStatuses(projectPath);
    return Object.fromEntries(statuses);
  });
  electron.ipcMain.handle("git:branch", async (_, projectPath) => {
  ...
  electron.ipcMain.handle("sessions:getRecent", async () => {
    return loadRecentSessions();
  });
  electron.ipcMain.handle("sessions:addRecent", async (_, projectPath) => {
    const sessions2 = await loadRecentSessions();
    const existing = sessions2.findIndex((s) => s.projectPath === projectPath);
    if (existing >= 0) sessions2.splice(existing, 1);
    sessions2.unshift({ projectPath, branch: null, lastOpened: Date.now() });
    if (sessions2.length > 20) sessions2.length = 20;
    await saveRecentSessions(sessions2);
  });
  electron.ipcMain.handle("pty:create", async (event, projectPath) => {
  ...
  electron.ipcMain.on("pty:write", (_, id, data) => {
    writePty(id, data);
  });
  electron.ipcMain.on("pty:resize", (_, id, cols, rows) => {
    resizePty(id, cols, rows);
  });
  electron.ipcMain.on("pty:destroy", (_, id) => {
    destroyPty(id);
  });
  electron.ipcMain.handle("watcher:start", async (event, projectPath) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender);
    if (!window) throw new Error("No window found");
...
function createWindow() {
  ...
  if (is.dev && process.env["ELECTRON_RENDERER_URL"]) {
    ...
    window.loadFile(sysPath.join(__dirname, "../renderer/index.html"));
  }
